...
fn splice_collection(base: &[u8], begin_rid: u32, end_rid: u32, record_rid: u32, extra_payloads: &[Vec<u8>]) -> Vec<u8> {
...
impl StylesBuilder {
    ...
    fn resolve_border(&mut self, fmt: &Format) -> u16 {
        ...
        self.border_records.push(encode_border(fmt.border_style, fmt.border_color));
    ...
    pub fn register(&mut self, fmt: &Format) -> u16 {
        ...
        let row = encode_xf(ifmt, ifont, ifill, iborder, fmt.halign as u8, fmt.valign as u8, fmt.text_wrap);
    ...
    pub fn build(&self) -> Vec<u8> {
        ...
        out = splice_collection(&out, RID_BEGIN_BORDERS, RID_END_BORDERS, RID_BORDER, &self.border_records);
